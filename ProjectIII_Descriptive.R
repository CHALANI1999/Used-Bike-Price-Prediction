...
table(df1$owner)
table(df1$state)
trainset=trainset %>%  filter(!row_number() %in% x)
table(trainset$brand_New)
table(trainset$owner)
table(trainset$state)
options(repr.plot.width=12,repr.plot.height=7)
...
ggplot(data = trainset, aes(x = power, y = sqrt(price))) +
  geom_point() +
  geom_smooth(data = tg4, aes(x = power, y = mean_price), method = "lm", color = "red", se = FALSE) +
  theme_minimal() +
  labs(x = "power cc", y = "square root transformed Bike Price")
...
plotVIPScores(ModelPLS,ncomp=5, type = "h",show.label = TRUE)
...
result = kruskal.test(age ~ brand_New,
                      data = trainset)
print(result)
result = kruskal.test(kms_driven ~ brand_New,
                      data = trainset)
print(result)

chisq.test(trainset$brand_New, trainset$owner, correct=FALSE)
chisq.test(trainset$brand_New, trainset$state, correct=FALSE)


options(repr.plot.width=12,repr.plot.height=7)
hist_boxplot(testset$price,main="Price distribution",col="#9494b8",xlab="Bike Price(In Indian Rupees)")
boxplot.stats(testset$price)$stats
x=which(testset$price %in% boxplot.stats(testset$price)$out)
length(x)
df1=testset %>%  filter(row_number() %in% x)
View(df1)
table(df1$brand_New)
table(df1$owner)
table(df1$state)
testset1=testset %>%  filter(!row_number() %in% x)
table(testset1$brand_New)
table(testset1$owner)
table(testset1$state)
