...
hist_boxplot(trainset_new$kms_driven,main="Price distribution",col="#9494b8",xlab="Bike Price(In Indian Rupees)")
boxplot.stats(trainset_new$kms_driven)$stats
max(trainset_new$kms_driven)
df=data[data$kms_driven<10,]
View(df)
y=which(data$kms_driven<10)
data=data %>%  filter(!row_number() %in% as.numeric((y)))
nrow(data)
options(repr.plot.width=12,repr.plot.height=7)
hist_boxplot(trainset_new$age,main="Price distribution",col="#9494b8",xlab="Bike Price(In Indian Rupees)")
boxplot.stats(trainset_new$age)$stats
max(trainset_new$age)
df=data[data$age>20,]
View(df)

options(repr.plot.width=12,repr.plot.height=7)
hist_boxplot(trainset_new$power,main="Price distribution",col="#9494b8",xlab="Bike Price(In Indian Rupees)")
boxplot.stats(trainset_new$power)$stats
max(trainset_new$power)


ggplot(data=trainset, aes(x=kms_driven, y=log(price))) +
  geom_point() +
  theme_minimal() +
...
ggplot(data=trainset,aes(x=log(power),y=log(price),fill=brand_New,color=brand_New))+
  geom_point()+
...
#####################################################################################
dummy_coding= dummyVars(" ~ . ", data = testset_new)
testset_encoded=predict(dummy_coding, newdata =testset_new)

View(testset_encoded)

x=testset_encoded[,-1]
x[,c(1,6,7)]=scale(x[,c(1,6,7)], center = TRUE, scale = TRUE)
y=as.matrix(testset_encoded[,1])
set.seed(100)
ModelPLS = pls(x,y,cv=5, info = "Bike Price prediction")
#view summary of model fitting
summary(ModelPLS)
...
plotXScores(ModelPLS,show.label = FALSE)
plotXYLoadings(ModelPLS,show.label = FALSE)
plotVIPScores(ModelPLS,ncomp=5, type = "h",show.label = FALSE)
summary(ModelPLS$coeffs)
plot(ModelPLS$coeffs,show.label = TRUE)
summary(ModelPLS$res$cal)
#Checking Outliers
...
# Identify outlier indices
# get row indices for outliers in calibration set
outliers1 = which(categorize(Model0, ModelPLS$res$cal) == "outlier")
length(outliers1)
df2=testset_new[c(outliers1),]
View(df2)
table(df2$brand_New)
table(df2$owner)
table(df2$state)
testset_new=testset_new %>%  filter(!row_number() %in% outliers1)
str(trainset_new)
table(trainset_new$owner)
table(testset_new$owner)
table(trainset_new$brand_New)
...
nrow(trainset_new)
nrow(testset_new)
cor_test_result1 = cor.test(trainset_new$kms_driven, trainset_new$age)
cor_test_result2 = cor.test(trainset_new$kms_driven, trainset_new$power)
cor_test_result3 = cor.test(trainset_new$power, trainset_new$age)
...
mape_m = mape(y1, y_hat_ridge1)
rmse_m = rmse(y1, y_hat_ridge1)
mape_m
rmse_m
mape_m = mape(y1, y_hat_ridge2)
...
mape_m = mape(y1, pred1)
rmse_m = rmse(y1,pred1)
mape_m
rmse_m
mape_m = mape(y1, pred2)
...
#################################REGULARIZATION WITH IMPORTANT FEATURES########################################
data1=trainset_new[,-1]
data$owner= factor(data$owner,level=c("First Owner","Second Owner","Third Owner",
                                      "Fourth Owner Or More"))
data$brand_New = factor(data$brand_New,level=c("Bajaj","Benelli","BMW","Ducati","Harley-Davidson","Hero","Honda",
                                               "Hyosung","Kawasaki","KTM","Mahindra","Royal Enfield"
...
dummy_1= dummyVars(" ~ . ", data = data1,sep = "")
data1_encoded=predict(dummy_1, newdata =data1)
vars_imp=c("price","power","age","brand_NewHarley-Davidson","kms_driven","brand_NewTriumph","brand_NewRoyal Enfield","brand_NewKTM","brand_NewKawasaki"
           ,"brand_NewDucati","brand_NewHyosung","brand_NewBenelli","brand_NewSuzuki","brand_NewYamaha","stateKarnataka","stateMaharashtra","stateDelhi","brand_NewHonda"
           ,"brand_NewBMW","ownerSecond Owner","brand_NewHero")
View(data1_encoded)
data1_imp = subset(data1_encoded,select=vars_imp)
View(data1_imp)
colnames(data1_imp)
data1_imp=as.data.frame(data1_imp)
# Convert dummy variables to factors
...
data1_imp=as.data.frame(data1_imp)
str(data1_imp)
data1_imp[,c(2,3,5)]=scale(data1_imp[,c(2,3,5)], center = TRUE, scale = TRUE)
data1_imp[,2]=as.numeric(data1_imp[,2])
data1_imp[,3]=as.numeric(data1_imp[,3])
data1_imp[,5]=as.numeric(data1_imp[,5])
model_matrix = model.matrix(price~., data = data1_imp)[,-1]
...
dummy_1= dummyVars(" ~ . ", data = data1,sep = "")
data1_encoded=predict(dummy_1, newdata =data1)
vars_imp=c("sq_price","power","age","brand_NewHarley-Davidson","kms_driven","brand_NewTriumph","brand_NewRoyal Enfield","brand_NewKTM","brand_NewKawasaki"
           ,"brand_NewDucati","brand_NewHyosung","brand_NewBenelli","brand_NewSuzuki","brand_NewYamaha","stateKarnataka","stateMaharashtra","stateDelhi","brand_NewHonda"
           ,"ownerSecond Owner","brand_NewHero","stateHaryana","stateUttar Pradesh","stateTelangana","stateGujarat","brand_NewTVS","stateTamil Nadu","brand_NewBMW"
           ,"ownerThird Owner","stateWest Bengal","brand_NewMahindra","stateRajasthan")
View(data1_encoded)
data1_imp = subset(data1_encoded,select=vars_imp)
View(data1_imp)
colnames(data1_imp)
...
data1_imp[,3]=as.numeric(data1_imp[,3])
data1_imp[,5]=as.numeric(data1_imp[,5])
model_matrix = model.matrix(sq_price~., data = data1_imp)[,-1]

data2=testset_new[,-2]


dummy_2= dummyVars(" ~ . ", data = data2,sep = "")
data2_encoded=predict(dummy_2, newdata =data2)
vars_imp=c("sq_price","power","age","brand_NewHarley-Davidson","kms_driven","brand_NewTriumph","brand_NewRoyal Enfield","brand_NewKTM","brand_NewKawasaki"
           ,"brand_NewDucati","brand_NewHyosung","brand_NewBenelli","brand_NewSuzuki","brand_NewYamaha","stateKarnataka","stateMaharashtra","stateDelhi","brand_NewHonda"
           ,"ownerSecond Owner","brand_NewHero","stateHaryana","stateUttar Pradesh","stateTelangana","stateGujarat","brand_NewTVS","stateTamil Nadu","brand_NewBMW"
           ,"ownerThird Owner","stateWest Bengal","brand_NewMahindra","stateRajasthan")
View(data2_encoded)
data2_imp = subset(data2_encoded,select=vars_imp)
View(data2_imp)
colnames(data2_imp)
data2_imp=as.data.frame(data2_imp)
# Convert dummy variables to factors
...
data2_imp=as.data.frame(data2_imp)
str(data2_imp)
data2_imp[,c(2,3,5)]=scale(data2_imp[,c(2,3,5)], center = TRUE, scale = TRUE)
data2_imp[,2]=as.numeric(data2_imp[,2])
data2_imp[,3]=as.numeric(data2_imp[,3])
data2_imp[,5]=as.numeric(data2_imp[,5])
model_matrix1 = model.matrix(sq_price~., data = data2_imp)[,-1]
...
# Check multiple R-squared
y_hat_enet = predict(elastic_net_model, data1_imp[,-1])
actual=y1
pred=c(y_hat_enet)
rmse=sqrt(sum((actual -pred)^2)/nrow(data1))
d_actual=trainset_new$price
...
y_hat_enet = predict(elastic_net_model, data2_imp[,-1])
actual=y1
pred=c(y_hat_enet)
rmse=sqrt(sum((actual -pred)^2)/nrow(data1))
d_actual=testset_new$price
